...
- **Spawn** — launching a context-aware Claude Code session on the Mac mini session server for a task, via the `/spawn` skill. Takes a Todoist task ID or free-form description, loads the right project directory and CLAUDE.md, crafts a focused brief, moves the task to In-Progress, and opens a WinsomeChat window (falls back to a local iTerm2 session if the server is unreachable).
